...
if [ ! -n "$args" ]; then
    echo "Usage: build-asciidocs.sh git-tags [version]"
    exit 1
fi
...
do
  ...
  GIT_TAG="$git_tag"
  git clone https://github.com/agrestio/agrest-docs.git "AGREST_TMP_DIR_VERSION" --branch "$GIT_TAG"
  ...
  for d in */ ; do
      ...
      cp -R "./${d}target/site/$GIT_TAG." "$ASCII_DOC_DIR_VERSION/"
  done
